...
const recommendPloggingPlace = (userQuery) => {
  ...
  if (courseByName) {
    for (const district in plogging_places) {
      if (plogging_places[district][courseByName]) {
        const distance = plogging_places[district][courseByName].distance;
  ...
  if (selectedDistrict) {
    ...
    const distance = selectedDistrict[randomPlace].distance;
